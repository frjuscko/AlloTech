...
ini_set('display_startup_errors', 1);
    ...
    if (isset($_POST['submit'])) {
        ...
        /* Gestion du fichier */
        $photo_path = null;
        $uploadDir = '../uploads/villes/';  // ✅ Ajout du slash à la fin
        ...
        if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($file_type, $allowed_types)) {
                header('Location: ../villeadd.php?error=type_invalide');
                exit;
            }
            ...
            if ($file_size > $max_size) {
                header('Location: ../villeadd.php?error=fichier_trop_grand');
                exit;
            }
            
            // Générer un nom unique pour l'image
            $extension = pathinfo($file_name, PATHINFO_EXTENSION);
            $new_filename = 'ville_' . uniqid() . '_' . time() . '.' . $extension;
            ...
            if (move_uploaded_file($file_tmp, $destination)) {
                $photo_path = 'uploads/villes/' . $new_filename;
            } else {
                header('Location: ../villeadd.php?error=upload_echoue');
                exit;
            }
            ...
            header('Location: ../villeadd.php?error=' . urlencode($error_msg));
            exit;
        }

        try {
            $sql = "INSERT INTO villes (code, nom, photo, date_creation) VALUES (:code, :nom, :photo, NOW())";
            ...
            header("Location: ../villes.php?message=" . urlencode($message) . "&type=success");
            exit;
